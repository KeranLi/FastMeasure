...
class GrainPlot(object):
    ...
    def __init__(self,
        ...
        show_labels : bool, default True. Whether to display grain labels/numbers next to each grain.
        kwargs : dict
            Keyword arguments to pass to plt.figure().
        '''
        logger.info('Creating GrainPlot...')

        # 关键修复：从kwargs中弹出show_labels，防止传递给plt.figure()
        # 即使它已经在签名中定义，也要确保不会进入kwargs
        show_labels = kwargs.pop('show_labels', True)
        
        # Store color palette and color_by selection for later use
        self.color_palette = color_palette
        self.color_by = color_by
        self.show_labels = show_labels  # 保存到实例
    ...
    def _draw_label(self, grain: Grain, index: int):
        '''
        在颗粒质心位置绘制编号标签
        ...
        '''
        # 确保颗粒已被测量
        data = grain.data if grain.data is not None else grain.measure()
        
        # 获取质心坐标（注意：data['centroid-0']是y，data['centroid-1']是x）
        centroid_x = data['centroid-1'] * self.scale
        centroid_y = data['centroid-0'] * self.scale
        
        # 调试日志
        logger.debug(f"Drawing label {index + 1} at ({centroid_x:.1f}, {centroid_y:.1f})")
        
        # 创建带白色背景的文本标签
        text = self.ax.text(
            centroid_x, centroid_y,
            str(index),  # 显示为based索引
            fontsize=8,
            ha='center', 
            va='center',
            color='black',
    ...
    def calibrate_scale_from_grain(self, grain_index: int, real_length_m: float, 
                                   axis: str = 'major'):
        """
        根据已知尺寸的参考颗粒（硬币）校准比例尺
        ...
        real_length_m : float
            该颗粒的真实长度（单位：米）
        axis : str, default 'major'
            使用哪个轴作为参考：'major'（长轴）或 'minor'（短轴）
            
        Examples
        --------
        # 如果硬币#5的直径是25mm
        plot.calibrate_scale_from_grain(5, 0.025, axis='major')
        
        # 如果已知颗粒#10的短轴是10mm
        plot.calibrate_scale_from_grain(10, 0.010, axis='minor')
        """
        if grain_index < 0 or grain_index >= len(self.grains):
            logger.error(f"颗粒索引 {grain_index} 超出范围 [0, {len(self.grains)-1}]")
            return
        
        ref_grain = self.grains[grain_index]
        if ref_grain.data is None:
            ref_grain.measure()
        
        # 获取像素长度
        if axis == 'major':
            pixel_length = ref_grain.data['major_axis_length']
        elif axis == 'minor':
            pixel_length = ref_grain.data['minor_axis_length']
        else:
            logger.error("axis 参数必须是 'major' 或 'minor'")
            return
        
        # 计算比例尺 (pixels per meter)
        self.px_per_m = pixel_length / real_length_m
        
        # 同时更新scale_m以便scale bar正确显示
        self.scale_m = real_length_m
        
        logger.info(f"✓ 比例尺校准成功：{self.px_per_m:.3f} pixels/meter")
        logger.info(f"✓ 参考颗粒 #{grain_index}: {pixel_length:.2f} px = {real_length_m*1000:.2f} mm")
        
        # 清除所有颗粒的测量缓存，下次访问时会自动用新比例尺重新计算
        for grain in self.grains:
            grain.data = None
        
        # 重绘scale bar（如果存在）
        if hasattr(self, 'scale_line') and self.scale_line is not None:
            self._redraw_scale_bar()
        
        # 更新info box的单位显示（如果当前有颗粒被选中）
        if self.info_grain is not None:
    ...
    def merge_grains(self) -> Grain:
        ...
        # 为新颗粒添加标签
        if self.show_labels:
            self._draw_label(new_grain, len(self.grains) - 1)
            # 重建artists列表
            self.artists = [self.info,
                            *self.box_selector.artists,
                            *self.text_labels]
