...
# 导入UltraFastSAM系统
from rock_fastsam_system import RockUltraSystem


def parse_arguments():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(
        description="UltraFastSAM岩石颗粒自动分割系统",
...
使用示例:
  ...
  python run_fastsam.py --input image.tif --quiet
    ...
    parser.add_argument(
        "--version", "-v",
        action="version",
        version="UltraFastSAM岩石颗粒自动分割系统 v2.0.0"
    )
...
def print_welcome():
    ...
    print("=" * 70)
    print("功能：终极优化的岩石显微图像颗粒分割")
    print("版本：2.0.0（解决所有FastSAM问题）")
    print("特点：高召回率、智能后处理、完全兼容项目一")
    print("=" * 70)
